...
import { createUserSchema, searchUserSchema } from '@shared/dtos';
...
export class UserTrpcRouter {
  ...
  routes = router({
    ...
    findAll: procedure.input(searchUserSchema).query(async (opts) => {
      return this.userService.findAll(opts.input);
    }),
